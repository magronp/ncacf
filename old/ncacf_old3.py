...
def train_ncacf(params, path_pretrain=None, variant='relaxed', inter='mult', out_act='sigmoid'):
    ...
    # Get the number of songs and users
    n_users = len(open(params['data_dir'] + 'unique_uid.txt').readlines())
    n_songs_train = int(0.8 * 0.9 * len(open(params['data_dir'] + 'unique_sid.txt').readlines()))
...
def train_ncacf_new(params, path_pretrain=None, setting='cold', variant='relaxed', inter='mult'):
    ...
    for ep in range(params['n_epochs']):
        ...
        for data in tqdm(my_dataloader, desc='Training', unit=' Batches(s)'):
            ...
            it = data[2].to(params['device'])
            # Forward pass
            pred_rat, w_gmf, w_mlp, h_gmf, h_mlp, h_con = my_model(u_total, x, it)
            # Back-propagation
            loss = wpe_joint_ncacfnew(count_i, torch.transpose(pred_rat, 1, 0), w_gmf, h_gmf, w_mlp, h_mlp, h_con, lW, lH)
            #loss = wpe_joint(count_i, torch.transpose(pred_rat, 1, 0), w, h, h_con, lW, lH)
            loss.backward()
